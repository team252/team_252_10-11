...
#include "defines.h"
#include "motor.h" //Include the motor management library
/////////////////////////////////////////////////////////////////////////////////////////////////////
//
...
{
  bMotorFlippedMode[LEFT_REAR_POINTER] = true;
  return;
}
...
task main()
{
  ...
  while (true)//loop the code
  {
    ...
    }else{//if we should be stopped
      setMotor(LEFT_REAR, 0);//stop
    }
    if(abs(joystick.joy1_y2) > 10){
      setMotor(RIGHT_REAR, joystick.joy1_y2*100.0/127);//set right REAR to percent of joystick

    }else{//if we should be stopped
      setMotor(RIGHT_REAR, 0);//stop
    }
    if(abs(joystick.joy2_y1) > 10){//when the joystick is above 10 wither way

      setMotor(RAMP_WHEELS, joystick.joy2_y1*100.0/127); //set ramp wheels to percent of joystick
    }else{//if we should be stopped
      setMotor(RAMP_WHEELS, 0);//stop
    }
